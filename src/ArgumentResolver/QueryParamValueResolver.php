...
class QueryParamValueResolver implements ArgumentValueResolverInterface, LoggerAwareInterface
{
    ...
    public function resolve(Request $request, ArgumentMetadata $argument): iterable
    {
        ...
        if (is_null($value = $request->query->get($name))) {
            try {
                if (in_array($name, array_keys($request->toArray()))) {
                    $value = $request->toArray()[$name];
                }
            } catch (\Exception $e) {
                $this->logger->info("Request body parse error: '" . $e->getMessage() . "'");
            }
        }
